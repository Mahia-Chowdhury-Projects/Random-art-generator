...
for (6, 5, (fn x => (print ((Int.toString(x)) ^ "\n"))));
...
fun build (depth, rand: RandomGenerator) =
   ...
   let 
        val pick1 = rand(2,8)
    in
        if pick1 = 2 then buildSine(build(depth-1, rand))
        else
        if pick1 = 3 then buildCosine(build(depth-1, rand))
        else
        if pick1 = 4 then buildAverage(build(depth-1, rand), build(depth-1, rand))
        else
            if pick1 = 5 then buildTimes(build(depth-1, rand), build(depth-1, rand))
            (* Extensions *)
        else
            if pick1 = 6 then buildAvgThree(build(depth-1, rand), build(depth-1, rand), build(depth-1, rand))
        else
            if pick1 = 7 then buildAbsoluteVal(build(depth-1, rand))
        else
            buildNegation(build(depth-1, rand))
        end; 
...
build (6, makeRand(12,18)); 
build(5, makeRand(1,2));
build(0, makeRand(3,4));


doRandomGray(8, 50, 98);

doRandomColor(8,50,98);
